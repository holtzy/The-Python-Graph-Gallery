...
const mergeTypes = ({
  ...
  const isSafeMerge = !plugin || plugin.name === `default-site-plugin` || plugin.name === typeOwner || isOverridableBuiltInType;
  ...
  Object.keys(childNodesByType).forEach(typeName => {
    ...
    if (shouldInfer === false) {
      ...
      if (!childOfExtension || !childOfExtension.types.includes(parentTypeName) || !childOfExtension.many === many) {
        ...
        report.warn(`Deprecation warning: ` + `In Gatsby v3 field \`${parentTypeName}.${fieldName}\` will not be added automatically because ` + `type \`${typeName}\` does not explicitly list type \`${parentTypeName}\` in \`childOf\` extension.\n` + `Add the following type definition to fix this:\n\n` + `  type ${typeName} implements Node @childOf(types: ["${parentTypeName}"]${manyArg}) {\n` + `    id: ID!\n` + `  }\n\n` + `https://www.gatsbyjs.com/docs/actions/#createTypes`);
      }
    }
