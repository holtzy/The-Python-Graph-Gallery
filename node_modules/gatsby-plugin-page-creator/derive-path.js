...
function derivePath(path, node, reporter) {
  ...
  slugParts.forEach(function (slugPart) {
    ...
    var value = safeSlugify(nodeValue); // 3.d  replace the part of the slug with the actual value
...
function safeSlugify(nodeValue) {
  ...
  var SLASH_PRESERVING_STATIC_KEY = "-replaced-";
  var replaceSlashesValue = (nodeValue + "").replace(/\//g, SLASH_PRESERVING_STATIC_KEY);
  var slugifiedWithoutSlashesValue = (0, _slugify.default)(replaceSlashesValue);
  return slugifiedWithoutSlashesValue.replace(new RegExp(SLASH_PRESERVING_STATIC_KEY, "g"), "/");
}